<div class="btn-group">
		...
		{if $player.playerLevel}<tr><td>{$player.playerRank}.</td><td>{$player.playerLevel} {include file="ranking_rankup.tpl"}</td><td>{$player.playerName}</td><td>{$player.playerExperience}</td><td>{if $player.playerProgress.progressExp != 0}{$player.playerProgress.progressExp}{else}-{/if}</td></tr>{/if}
		{/foreach}
		</tbody>
		</table>